...
function reset() {
    ...
    for (var i = 0; i < 9; i++) numCount[i] = 0;
...
function createGrid() {
    ...
    for (var i = 0; i < 9; i++) {
        ...
        for (var j = 0; j < 9; j++) grid[i][j] = new Cell();
    }
}
...
function setupGrid() {
    ...
    for (var i = 0; i < 9; i++) {
        for (var j = 0; j < 9; j++) {
            if (initVals[i][j] == 0) grid[i][j].writable = true;
            else {
...
function hintExpose() {
    ...
    for (var i = 0; i < 9; i++) {
        for (var j = 0; j < 9; j++) {
            if (grid[i][j].value == finalVals[i][j]) continue;

            if (grid[i][j].value == 0) emptyCount++;
            else if (grid[i][j].value != finalVals[i][j]) wrongCount++;
        }
    }
...
$(document).click(function (event) {
    ...
    // erase button
        ...
        for (var i = 0; i < 9; i++) cell.guess[i] = false;
